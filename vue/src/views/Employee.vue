    ...
    <el-dialog title="管理员信息" v-model="data.formVisible" width="500">
      <el-form :model="data.form" label-width="80px">
        ...
        <el-form-item label="介绍">
          <el-input v-model="data.form.description" type="textarea"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
const save = () => {
  if (data.form.id) {
    // 编辑操作
    request.put('employee/update', data.form)
        .then(res => {
          if (res.code === 200) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm('是否确认删除选中的数据？', '批量删除确认', { type: 'warning' }).then(() => {
    request.delete('employee/deleteBatchDelete', {data: data.ids}).then(res => {
      if (res.code === 200) {
